...
async function sendMessage(text) {
  ...
  // Find the bot (searches for "PenSellerBot" or you can change this)
  const dialogs = await client.getDialogs({ limit: 50 });
  const botDialog = dialogs.find(d => (d.title || d.name || '').includes('PenSellerBot'));

  if (!botDialog) {
    console.error('❌ Bot not found. Available dialogs:');
