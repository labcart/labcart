 ...
 */
function generateMcpConfig(profile = 'with-image-tools') {
...
async function sendSimple(options) {
  const { message, sessionId, claudeCmd, onStream, messageContent, onToolResult, timeout = 120000, workspacePath } = options;

  return new Promise((resolve, reject) => {
    let timeoutId = null;
    // Use no-image-tools profile for regular text conversations
    // This prevents accidental image tool calls during normal chat
    const mcpConfigPath = generateMcpConfig('no-image-tools');
...
async function sendToClaudeWithTTS(options) {
  const {
    ...
    botId,
    telegramUserId,
    onTurn2Start = null,  // Callback when Turn 2 starts (audio generation)
  ...
  return new Promise((resolve, reject) => {
    let timeoutId = null;
    let completed = false;
    // Use with-image-tools profile for TTS (TTS needs router with TTS tools)
    // Note: We use 'with-image-tools' which actually means "full tools" (includes TTS)
    const mcpConfigPath = generateMcpConfig('with-image-tools');

    const args = [
      '--ide',
      '--input-format', 'stream-json', // CRITICAL: Required for multi-turn
      '--output-format', 'stream-json',
      '--verbose',
      '--dangerously-skip-permissions',
      // POC: Use lightweight MCP Router instead of loading all MCPs
    ...
    console.log(`🚀 Spawning (TTS mode): ${claudeCmd} ${args.join(' ')}`);
    ...
    if (botId && telegramUserId && options.chatId && options.statusMsgId) {
      trackRequest(botId, telegramUserId, {
        ...
        console.warn('⚠️  Failed to track TTS request:', err.message);
      });
    }
    ...
    let sessionInfo = null;
    let audioData = null;
    let audioFilename = null; // Track the expected audio filename
    let turn = 1; // Track which turn we're on
    ...
    const resetInactivityTimeout = () => {
      ...
      timeoutId = setTimeout(() => {
        if (!completed) {
          console.error(`⏱️  TTS inactivity timeout after ${timeout}ms - no output from Claude process ${claudePid}`);
          try {
            child.kill('SIGTERM');
            killMcpChildren(claudePid);
          } catch (err) {
            console.error(`⚠️  Error killing timed-out TTS process:`, err.message);
          }
          reject({ success: false, error: `TTS timeout after ${timeout / 1000} seconds`, timeout: true });
        }
      }, timeout);
    };
    ...
    rl.on('line', (line) => {
      ...
      try {
        ...
        // DEBUG: Log ALL events during Turn 2
        if (turn === 2) {
          console.log(`🔍 [Turn 2] type=${data.type}${data.tool_name ? ', tool=' + data.tool_name : ''}`);
        }
        ...
        switch (data.type) {
          ...
          case 'tool_result':
            // Check if this is a TTS tool result (from turn 2)
            if (data.tool_name === 'mcp__tts__text_to_speech' && data.result) {
              try {
                const ttsResult = typeof data.result === 'string' ? JSON.parse(data.result) : data.result;
                if (ttsResult.audioBase64) {
          ...
          case 'result':
            if (turn === 1) {
              // Turn 1 complete - now request TTS
              console.log(`✅ Turn 1 complete (${data.duration_ms}ms) - text response received`);
              console.log(`🎤 Requesting TTS generation...`);

              turn = 2;

              // Generate structured filename for audio file
              audioFilename = `bot-${botId}-user-${telegramUserId}-${Date.now()}`;

              // Send turn 2: Request TTS tool call in JSON format
              const providerParam = ttsProvider ? `- provider: "${ttsProvider}"\n` : '';
...
Only call the tool, no additional response needed.`;
              ...
              // Notify that Turn 2 (audio generation) is starting
              if (onTurn2Start) {
                try {
                  onTurn2Start();
                } catch (err) {
              ...
              const turn2Message = JSON.stringify({
                ...
                message: {
                  role: 'user',
                  content: ttsRequestText
                }
              });
              child.stdin.write(turn2Message + '\n');

            } else if (turn === 2) {
              // Turn 2 complete - find and organize the audio file
              completed = true;
              if (timeoutId) clearTimeout(timeoutId);
              console.log(`✅ Turn 2 complete (${data.duration_ms}ms)`);

              // Look for the audio file that was created
              const audioOutputDir = path.join(process.cwd(), 'audio-output');
              const expectedFilename = `${audioFilename}-`; // Prefix to match (MCP adds timestamp suffix)

              try {
                // Find the newest file matching our filename pattern
                const files = fs.readdirSync(audioOutputDir)
                  .filter(f => f.startsWith(expectedFilename) && f.endsWith('.mp3'))
                  .map(f => ({
                    name: f,
                    path: path.join(audioOutputDir, f),
                    time: fs.statSync(path.join(audioOutputDir, f)).mtime
                  }))
                ...
                if (files.length > 0) {
                  ...
                  const newPath = path.join(organizedDir, audioFile.name);
                  fs.renameSync(audioFile.path, newPath);

                  console.log(`🎵 Audio file organized: ${newPath}`);
                  ...
                  console.error(`⚠️  Audio file not found with pattern: ${expectedFilename}`);
                ...
                console.error(`⚠️  Error organizing audio file:`, err.message);
    ...
    // Send turn 1: User message in JSON format
    // Use messageContent if provided (for structured content like images), otherwise use message text
...
async function sendToClaudeWithImage(options) {
  const { message, messageContent, userText, sessionId, claudeCmd, botId, telegramUserId, imageModel, imageSize, imageQuality, imageStyle, imagePromptContext, onTurn2Start } = options;
  ...
  return new Promise((resolve, reject) => {
    ...
    if (sessionId) {
      ...
      mcpConfigPath = generateMcpConfig('with-image-tools');
      cmd = `${claudeCmd} --ide --resume ${sessionId} --input-format stream-json --output-format stream-json --verbose --dangerously-skip-permissions --strict-mcp-config --mcp-config ${mcpConfigPath}`;
      ...
      mcpConfigPath = generateMcpConfig('no-image-tools');
      cmd = `${claudeCmd} --input-format stream-json --output-format stream-json --verbose --dangerously-skip-permissions --strict-mcp-config --mcp-config ${mcpConfigPath}`;
